...
int main (int argc, char **argv){
  ...
  int min_size, max_size, iterations, nsamples;
  ...
  string output_file = "../timings/MC4/";
  string out_files[nparenth];
  std::ofstream ofiles[nparenth];

  if (argc != 9){
    cout << "Execution: " << argv[0] << " min_size max_size iterations nsamples"
      " lo_margin up_margin cube_file output_file" << endl;
    return (-1);
  } else {
    ...
    lo_margin = atof (argv[5]);
    up_margin = atof (argv[6]);
    cube_filename.append (argv[7]);
    output_file.append (argv[8]);
  }
  ...
  printf("TOTAL Computing time: %f\n", std::chrono::duration<double>(start - end).count());
