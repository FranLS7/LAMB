/**
 * @file profile_SYMM.cpp
 * @author FranLS7 (flopz@cs.umu.se)
 * @brief This program is used to generate performance profile data for SYMM
 * @version 0.1
 * @date 2022-01-21
 * 
...
std::vector<unsigned long> getFlopsSYMM (const iVector2D &points);

dVector2D explore2DSYMM(const iVector2D &points, const int iterations, const int n_threads);
...
int main(int argc, char **argv) {
  ...
  std::vector<unsigned long> flops = getFlopsSYMM(points);

  dVector2D result = explore2DSYMM(points, iterations, n_threads);

  // write to files
  ofile.open(root_dir + std::string("symm.csv"));
...
dVector2D explore2DSYMM(const iVector2D &points, const int iterations, const int n_threads) {
  ...
  for (const auto &point : points) {
    ...
    // }
    // std::cout << "}\n";
    A = (double*)mkl_malloc(point[0] * point[0] * sizeof(double), ALIGN);
    B = (double*)mkl_malloc(point[0] * point[1] * sizeof(double), ALIGN);
    C = (double*)mkl_malloc(point[0] * point[1] * sizeof(double), ALIGN);

    for (int i = 0; i < point[0] * point[0]; i++) A[i] = drand48();
    for (int i = 0; i < point[0] * point[1]; i++) B[i] = drand48();

    for (int it = 0; it < iterations; ++it) {
      for (int i = 0; i < point[0] * point[1]; i++) C[i] = drand48();
      lamb::cacheFlush(n_threads);
      lamb::cacheFlush(n_threads);
      lamb::cacheFlush(n_threads);

      auto time1 = std::chrono::high_resolution_clock::now();
      cblas_dsymm(CblasColMajor, CblasLeft, CblasUpper, point[0], point[1], 1.0, A, point[0], 
      B, point[0], 1.0, C, point[0]);
...
std::vector<unsigned long> getFlopsSYMM (const iVector2D &points) {
  ...
  for (auto const &point : points)
    flops.push_back(static_cast<unsigned long>(2 * point[0] * point[0] * point[1]));
