...
int main (int argc, char **argv){
  ...
  M1 = (double*)malloc(d3 * d5 * sizeof(double));
  M2 = (double*)malloc(d2 * d5 * sizeof(double));
  ...
  cache_flush();
  dgemm_(&transpose, &transpose, &d3, &d5, &d4, &one, C, &d3, D, &d4, &one, M1, &d3);
  dgemm_(&transpose, &transpose, &d2, &d5, &d3, &one, B, &d2, M1, &d3, &one, M2, &d2);
  dgemm_(&transpose, &transpose, &d1, &d5, &d2, &one, A, &d1, M2, &d2, &one, X, &d1);
