...
int main (int argc, char **argv){
  ...
  M2 = (double*)malloc(d3 * d5 * sizeof(double));
  ...
  dgemm_(&transpose, &transpose, &d3, &d5, &d4, &one, C, &d3, D, &d4, &one, M2, &d3);
  dgemm_(&transpose, &transpose, &d1, &d5, &d3, &one, M1, &d1, M2, &d3, &one, X, &d1);
