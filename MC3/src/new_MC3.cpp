...
bool computation_decision (int *dims, int ndim, int threshold, double margin_flops,
  double margin_vflops, lamb::GEMM_Cube& cube);

inline double gemm_flops (int d0, int d1, int d2);
inline bool within_range (double x, double y, double margin);
...
int main (int argc, char **argv){
  ...
  int *dims, ndim = 4;
  int max_size, jump_size, npoints, iterations;
  double *times;

  int threshold = 20;
  double margin_flops = 0.3;
  ...
  npoints = int(max_size / jump_size);
    ...
    for (dims[1] = jump_size; dims[1] <= max_size; dims[1] += jump_size){
      ...
      for (dims[2] = jump_size; dims[2] <= max_size; dims[2] += jump_size){
        for (dims[3] = jump_size; dims[3] <= max_size; dims[3] += jump_size){
          ...
          // >> perhaps it's better to use while-loops instead.
          // auto time1 = std::chrono::high_resolution_clock::now();
          foo = computation_decision (dims, ndim, threshold, margin_flops, margin_flops, kubo);
...
bool computation_decision (int *dims, int ndim, int threshold, double margin_flops,
  ...
  if (compute)
    ...
    if (within_range (flops_0, flops_1, margin_flops)){
      ...
      if (within_range (v_flops_0, v_flops_1, margin_vflops))
        compute = true;
    }
  }
  return compute;
